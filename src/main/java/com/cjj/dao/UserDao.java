...
public class UserDao extends TemplateDao {
    public List<User> listAll(String name, String sex, Page page) {
        if ("-1".equals(sex)) {
            ...
            String sql = "SELECT " +
                    ...
                    "LIMIT ?,?";
            return template.query(sql, new BeanPropertyRowMapper<User>(User.class),
                    "%" + name + "%", sex, (page.getPageCurrent() - 1) * page.getSize(), page.getSize());
        }
    }
    ...
    public void add(User user) {
        ...
        template.update(sql, user.getUsername(), user.getPassword(), user.getEmail(), user.getAge(),
                user.getSex(), user.getDescription(), user.getDeptName(), user.getRegisterTime(),
                user.getRealName(), user.getWxOpenid(), user.getPic());
    }
